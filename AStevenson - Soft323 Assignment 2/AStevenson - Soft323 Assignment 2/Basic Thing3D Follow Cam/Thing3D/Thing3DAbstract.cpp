						   ...
						   ID3D11DeviceContext  *pImmediateContext)
{
   ...
   = cameraZoom    
   = cameraDistance
   = cameraRotation     = 0.0f; // 'z' key for zoom in and 'x' for zoom out
   rx = ry = rz         = 0.0f;
   sx = sy = sz         = 1.0f;
   ...
   lightDiffuseColour    = XMFLOAT4(0.2, 0.2, 0.2, 0.2);
...
void Thing3DAbstract::cleanObject()
{
	int debugNum = g_meshData.numIndices;
	//delete g_meshData.materials; - cant delete?
